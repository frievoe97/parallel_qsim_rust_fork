...
fn create_resources_with_plan(out_dir: &PathBuf, plan_file: &str) {
    ...
    store_to_file(&out_dir.join("ids.binpb"));
    net.to_file(&out_dir.join("equil-network.binpb"));
    pop.to_file(&out_dir.join("population.binpb"));
    garage.to_file(&out_dir.join("equil-vehicles.binpb"));
}
...
#[test]
#[serial]
fn teleport_network_route_no_main_mode() {
    let test_dir = PathBuf::from("./test_output/simulation/equil_no_main/");
    create_resources_with_plan(&test_dir, "equil-1-plan.xml");

    let config_args = CommandLineArgs {
        config_path: "./tests/resources/equil/equil-config-no-main.yml".to_string(),
        num_parts: None,
    };
    ...
    execute_sim(
        DummySimCommunicator(),
        Box::new(TestSubscriber::new_with_events_from_file(
            "./tests/resources/equil/expected_events_teleported.xml",
        )),
        config_args,
    );
}
...
#[test]
#[serial]
fn teleport_generic_route_no_main_mode() {
    let test_dir = PathBuf::from("./test_output/simulation/equil_no_main/");
    create_resources_with_plan(&test_dir, "equil-generic-plan.xml");
...
#[serial]
#[should_panic]
fn generic_route_main_mode_panics() {
    let test_dir = PathBuf::from("./test_output/simulation/equil_main/");
    create_resources_with_plan(&test_dir, "equil-generic-plan.xml");

    let config_args = CommandLineArgs {
        config_path: "./tests/resources/equil/equil-config-main.yml".to_string(),
        num_parts: None,
    };
